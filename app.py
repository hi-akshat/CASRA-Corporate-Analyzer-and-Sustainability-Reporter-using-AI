...
def main(start_data, end_data):
    ...
    st.set_page_config(page_title="CASRA", page_icon=icon_path,
    ...
    if company and company != "Select a Company":
        ...
        # Allow user some time to select the two dates -- hacky :D
        time.sleep(0.8)
        ...
        link_df = link_df[["DATE", "ARTICLE"]].to_markdown()
        ...
        if line_metric == "ESG Scores":
            ...
            if line_metric == "Overall Score":
                ...
                plot_df = pd.concat([tone_df, ind_tone_df]
                                    ).reset_index(drop=True)
            else:
        ...
        def color_f(f): return f"Company: {company.title()}" if f == company else (
        ...
        fig_3d = px.scatter_3d(embeddings, x="0", y="1", z="2",
                               ...
                               hover_name="company",
                               hover_data={
                                   c: False for c in embeddings.columns},
                               )
